...
std::string to_string(uint8_t card) {
  ...
  return {kRanks[card / NUMBER_OF_SUITS], kSuits[card % NUMBER_OF_SUITS]};
}
...
TEST_F(FiveEvalTest, HighCard) {
  ...
  for (int i = 5; i < NUMBER_OF_FACES; ++i) {
    int const I = ((NUMBER_OF_FACES - 1 - i) << 2) + 1;  // No flushes.
    for (int j = 3; j < i; ++j) {
      int const J = (NUMBER_OF_FACES - 1 - j) << 2;
      for (int k = 2; k < j; ++k) {
        int const K = (NUMBER_OF_FACES - 1 - k) << 2;
        for (int l = 1; l < k; ++l) {
          int const L = (NUMBER_OF_FACES - 1 - l) << 2;
          for (int m = 0; m < l; ++m) {
            if (i - m == 4 || (i == NUMBER_OF_FACES - 1 && j == 3)) {
              continue;
            }
            int const M = (NUMBER_OF_FACES - 1 - m) << 2;
...
TEST_F(FiveEvalTest, Pair) {
  ...
  for (int i = 0; i < NUMBER_OF_FACES; ++i) {
    int const I = (NUMBER_OF_FACES - 1 - i) << 2;
...
TEST_F(FiveEvalTest, WorstStraightBeatsBestTwoPair) {
  ...
  ASSERT_EQ(rank_of_best_two_pair + 1, rank_of_worst_straight);
}
...
TEST_F(FiveEvalTest, WorstTripleBeatsBestStraight) {
  // Straight Ace high.
  ...
  ASSERT_EQ(rank_of_best_straight + 1, rank_of_worst_triple);
}
...
TEST_F(FiveEvalTest, WorstFullHouseBeatsBestTriple) {
  ...
  ASSERT_EQ(rank_of_best_triple + 1, rank_of_worst_full_house);
}
