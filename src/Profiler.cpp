 ...
 public:
  static double RandomAccessProfile(unsigned const count) {
    ...
    for (int i = 0; i < length; i += 28) {
      doNotOptimiseAway(SevenEval::GetRank(
          buffer[i + 21], buffer[i + 22], buffer[i + 23], buffer[i + 24],
          buffer[i + 25], buffer[i + 26], buffer[i + 27]));
      doNotOptimiseAway(SevenEval::GetRank(
...
int main() {
  ...
  if (fastest > 0.0) {
    ...
    std::cout << "Best random access rate: " << handsPerSecond << " hands/sec"
              << std::endl;
  }
}
